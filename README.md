...
* `noTargetSpecifiedError`: `-t, --target arg is empty, no target specified`: This error occurs when the `-t, --target` argument is
  given blank spaces or nothing at all (ex. `""`, `"     "`).
...
* `targetNotFoundError`: `Target 'NAME' was not found in DNR event data, double check and try again`. This occurs if CANDEW cannot
  find the target specified via the `-t, --target` argument. Double check that you entered the number/name correctly and try again.
